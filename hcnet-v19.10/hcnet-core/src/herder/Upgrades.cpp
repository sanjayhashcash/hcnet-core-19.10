...
save(Archive& ar, hcnet::Upgrades::UpgradeParameters const& p)
{
    ...
    {
        configUpgradeKeyStr = hcnet::decoder::encode_b64(
     ...
     hcnet::AbstractLedgerTxn& ltx)
{
    ...
    try
    {
        ...
        if (configUpgradeKeyStr)
        {
            ...
            hcnet::decoder::decode_b64(configUpgradeKeyStr.value(), buffer);
            hcnet::ConfigUpgradeSetKey key;
...
std::string
Upgrades::UpgradeParameters::toDebugJson(hcnet::AbstractLedgerTxn& ltx) const
{
    ...
    std::map<AccountID, std::vector<LedgerTxnEntry>> const& offersByAccount)
{
    ...
    for (auto const& accountOffers : offersByAccount)
    {
        ...
        auto accountEntry = hcnet::loadAccount(ltx, accountID);
        if (!accountEntry)
        {
...
upgradeFromProtocol15To16(AbstractLedgerTxn& ltx)
{
    if (gIsProductionNetwork)
    {
        ...
        auto seller = hcnet::loadAccountWithoutRecord(ltx, sellerID);
        auto offer = hcnet::loadOffer(ltx, sellerID, offerID);
