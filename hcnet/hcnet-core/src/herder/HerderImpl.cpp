...
void
HerderImpl::setTrackingSCPState(uint64_t index, HcnetValue const& value,
...
void
HerderImpl::newSlotExternalized(bool synchronous, HcnetValue const& value)
{
    ZoneScoped;
...
void
HerderImpl::processExternalized(uint64 slotIndex, HcnetValue const& value)
{
    ZoneScoped;
                             ...
                             std::optional<SecretKey> skToSignValue)
{
    ...
    HcnetValue sv =
        makeHcnetValue(txSet->getContentsHash(), closeTime, upgrades, sk);
...
HerderImpl::sendSCPStateToPeer(uint32 ledgerSeq, Peer::pointer peer)
{
    ...
    auto sendSlot = [weakPeer = std::weak_ptr<Peer>(peer)](SCPEnvelope const& e,
        ...
        m.type(SCP_MESSAGE);
        m.envelope() = e;
        auto mPtr = std::make_shared<HcnetMessage const>(m);
                              ...
                              bool checkTrackingSCP)
{
    ...
    // the exact closeTime contained in the HcnetValue with the best
    ...
    HcnetValue newProposedValue = makeHcnetValue(
...
HcnetValue
HerderImpl::makeHcnetValue(Hash const& txSetHash, uint64_t closeTime,
                             xdr::xvector<UpgradeType, 6> const& upgrades,
                             SecretKey const& s)
{
    ZoneScoped;
