...
struct xdr_fuzzer_compactor
{
    ...
    typename std::enable_if<
        (!std::is_same<hcnet::AccountID, T>::value &&
         !std::is_same<hcnet::MuxedAccount, T>::value &&
         !std::is_same<hcnet::Asset, T>::value &&
         !std::is_same<hcnet::AssetCode, T>::value &&
         !std::is_same<hcnet::ClaimableBalanceID, T>::value &&
         !std::is_same<hcnet::LedgerKey, T>::value) &&
    ...
    template <typename T>
    typename std::enable_if<std::is_same<hcnet::AccountID, T>::value>::type
    operator()(T const& pk)
    {
        // convert public key 1 byte
        check(1);
        auto b = hcnet::FuzzUtils::getShortKey(pk.ed25519());
        put_bytes(&b, 1);
    }
    template <typename T>
    typename std::enable_if<std::is_same<hcnet::MuxedAccount, T>::value>::type
    operator()(T const& m)
    {
        // convert MuxedAccount -> 1 byte (same than an AccountID)
        auto const& ed25519 = (m.type() == hcnet::KEY_TYPE_ED25519)
                                  ? m.ed25519()
                                  : m.med25519().ed25519;
        check(1);
        auto b = hcnet::FuzzUtils::getShortKey(ed25519);
        put_bytes(&b, 1);
    }

    template <typename T>
    typename std::enable_if<std::is_same<hcnet::Asset, T>::value>::type
    operator()(T const& asset)
    {
        // Convert Asset to 1 byte.
        check(1);
        auto b = hcnet::FuzzUtils::getShortKey(asset);
        put_bytes(&b, 1);
    }

    template <typename T>
    typename std::enable_if<std::is_same<hcnet::AssetCode, T>::value>::type
    operator()(T const& code)
    {
        // Convert AssetCode to 1 byte.
        check(1);
        auto b = hcnet::FuzzUtils::getShortKey(code);
        put_bytes(&b, 1);
    }

    template <typename T>
    typename std::enable_if<
        std::is_same<hcnet::ClaimableBalanceID, T>::value>::type
    operator()(T const& balanceID)
    {
        ...
        check(1);
        auto b = hcnet::FuzzUtils::getShortKey(balanceID);
        put_bytes(&b, 1);
    }

    template <typename T>
    typename std::enable_if<std::is_same<hcnet::LedgerKey, T>::value>::type
    operator()(T const& key)
    {
        ...
        check(1);
        auto b = hcnet::FuzzUtils::getShortKey(key);
        put_bytes(&b, 1);
    }
};
...
{
    hcnet::FuzzUtils::StoredLedgerKeys mStoredLedgerKeys;
    hcnet::FuzzUtils::StoredPoolIDs mStoredPoolIDs;
    std::uint8_t const* mCur;
    std::uint8_t const* const mEnd;

    xdr_fuzzer_unpacker(
        hcnet::FuzzUtils::StoredLedgerKeys const& storedLedgerKeys,
        hcnet::FuzzUtils::StoredPoolIDs const& storedPoolIDs,
    ...
    typename std::enable_if<
        (!std::is_same<hcnet::AccountID, T>::value &&
         ...
         !std::is_same<hcnet::AssetCode, T>::value &&
         !std::is_same<hcnet::ClaimableBalanceID, T>::value &&
         !std::is_same<hcnet::LiquidityPoolDepositOp, T>::value &&
         !std::is_same<hcnet::LiquidityPoolWithdrawOp, T>::value &&
         !std::is_same<hcnet::LedgerKey, T>::value) &&
        (xdr_traits<T>::is_class || xdr_traits<T>::is_container)>::type
    operator()(T& t)
    {
    ...
    operator()(T& pk)
    {
        ...
        hcnet::FuzzUtils::setShortKey(pk, v);
    }
    template <typename T>
    typename std::enable_if<std::is_same<hcnet::MuxedAccount, T>::value>::type
    operator()(T& m)
    {
        ...
        check(1);
        std::uint8_t v = get_byte();
        hcnet::FuzzUtils::setShortKey(m.ed25519(), v);
    }
    ...
    operator()(T& asset)
    {
        ...
        check(1);
        std::uint8_t v = get_byte();
        asset = hcnet::FuzzUtils::makeAsset(v);
    }
    ...
    operator()(T& code)
    {
        ...
        check(1);
        std::uint8_t v = get_byte();
        code = hcnet::FuzzUtils::makeAssetCode(v);
    }
    ...
    operator()(T& balanceID)
    {
        ...
        std::uint8_t v = get_byte();
        hcnet::LedgerKey key;
        hcnet::FuzzUtils::setShortKey(mStoredLedgerKeys, key, v);
        ...
        if (key.type() == hcnet::CLAIMABLE_BALANCE)
        {
            balanceID = key.claimableBalance().balanceID;
        }
        else
        {
            balanceID.type(hcnet::CLAIMABLE_BALANCE_ID_TYPE_V0);
            balanceID.v0()[0] = v;
        }
    }
    ...
    template <typename T>
    typename std::enable_if<
        std::is_same<hcnet::LiquidityPoolDepositOp, T>::value>::type
    operator()(T& depositOp)
    {
        check(1);
        auto v = get_byte();
        hcnet::FuzzUtils::setShortKey(mStoredPoolIDs,
        ...
        depositOp.minPrice = hcnet::Price{minN, minD};
        depositOp.maxPrice = hcnet::Price{maxN, maxD};
    }

    template <typename T>
    typename std::enable_if<
        std::is_same<hcnet::LiquidityPoolWithdrawOp, T>::value>::type
...
xdr_from_fuzzer_opaque(
    hcnet::FuzzUtils::StoredLedgerKeys const& storedLedgerKeys,
    hcnet::FuzzUtils::StoredPoolIDs const& storedPoolIDs, Bytes const& m,
...
generator_t::operator()(hcnet::PublicKey& t) const
{
    ...
    hcnet::FuzzUtils::setShortKey(
        t.ed25519(), static_cast<uint8_t>(hcnet::rand_uniform<int>(
...
TransactionFuzzer::storeSetupLedgerKeysAndPoolIDs(AbstractLedgerTxn& ltx)
{
    ...
    hcnet::FuzzUtils::generateStoredLedgerKeys(firstGeneratedLedgerKey,
...
TransactionFuzzer::reduceNativeBalancesAfterSetup(AbstractLedgerTxn& ltxOuter)
{
    ...
    for (auto const& param : accountParameters)
    {
        ...
        auto ae = hcnet::loadAccount(ltx, account);
    ...
    AbstractLedgerTxn& ltxOuter)
{
    ...
    for (auto const& trustLine : trustLineParameters)
    {
        ...
        // issuer.
        auto tle = hcnet::loadTrustLine(ltx, account, asset);
...
{
    reinitializeAllGlobalStateWithSeed(1);
    hcnet::FuzzUtils::generateStoredLedgerKeys(mStoredLedgerKeys.begin(),
                                                 mStoredLedgerKeys.end());
