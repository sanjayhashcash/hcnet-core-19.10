...
FeeBumpTransactionFrame::commonValidPreSeqNum(AbstractLedgerTxn& ltx)
{
    ...
    if (!hcnet::loadAccount(ltx, getFeeSourceID()))
    {
                                     ...
                                     AbstractLedgerTxn& ltxOuter, bool applying)
{
    ...
    auto feeSource = hcnet::loadAccount(ltx, getFeeSourceID());
    if (!checkSignature(
...
{
    auto account = hcnet::loadAccount(ltx, getFeeSourceID());
    if (!account)
    {
...
HcnetMessage
FeeBumpTransactionFrame::toHcnetMessage() const
{
    HcnetMessage msg(TRANSACTION);
